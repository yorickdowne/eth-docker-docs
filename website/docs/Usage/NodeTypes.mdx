...
You can run an Archive node, with all history and full lookup for all historical transactions. This needs at least a 4TB drive, and may need as much as 12TB, depending on the type of archive
and whether you are also archiving blobs.
...
You can run an Expired node, with rolling expiry history and receipts gone. This fits into a 2TB drive and is often the choice for validator nodes. This is the default.
...
- `pre-merge-expiry` - history since the merge, narrowly fits into 2TB but may soon need a 4TB drive
